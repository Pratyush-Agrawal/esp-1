...
store_data:
    ...
    unsigned int shift_right = VALUES_PER_WORD -1;
    unsigned int dma_length = 8 >> (shift_right);
    unsigned int chunk =  (blockidy  * output_cols +  blockidx ) <<3 ;
    ...
    dma_info_t &load_ctrl, dma_info_t &store_ctrl)
{
    ...
    const unsigned  incr = (conf_info_output_rows * conf_info_output_cols) >> (VALUES_PER_WORD-1) ;
    ...
    const unsigned dma_bound = store_offset_div + incr ;
