...
static int validate_buf(token_t *out, token_t *gold, int sys[])
{
        int i;
        int j;
        unsigned errors = 0;
        ...
        for (i = 0; i < 1; i++)
                for (j = 0; j < output_rows * output_rows; j++) {
                  if ((fabs(gold[j] - out[j]) / fabs(gold[j])) > ERR_TH) {
                                printf(" MISMATCH: GOLD = %d   and  OUT = %d   and SysC= %d and Element = %d \n", gold[j], out[j],sys[j], j);
                                errors++; }

                                else
                                printf("  GOLD = %d   and  OUT = %d  and  SysC= %d and Element = %d \n", gold[j], out[j],sys[j], j);
}
        return errors;
}
...
int main(int argc, char * argv[])
{
	...
	for (n = 0; n < ndev; n++) {
		...
		/* Validation */
		errors = validate_buf(&mem[out_offset], gold,sys);
#ifndef __riscv
		if (errors){
			printf("  ... FAIL\n");
			printf(" ERRORS = %d \n",errors);}
