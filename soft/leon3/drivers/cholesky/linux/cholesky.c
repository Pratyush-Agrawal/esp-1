...
static inline struct cholesky_device *to_cholesky(struct esp_device *esp)
{
	return container_of(esp, struct cholesky_device, esp);
}

static void cholesky_prep_xfer(struct esp_device *esp, void *arg)
{
	struct cholesky_access *a = arg;
...
{
	/* struct cholesky_device *cholesky = to_cholesky(esp); */
	/* struct cholesky_access *a = arg; */
...
{
	struct cholesky_device *cholesky;
	struct esp_device *esp;
	int rc;
...
static struct esp_driver cholesky_driver = {
	...
	.ioctl_cm	= CHOLESKY_IOC_ACCESS,
	.arg_size	= sizeof(struct cholesky_access),
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("cholesky driver");
